...
$email = $_POST["email"];
$fileRequired = $_POST["isMembership"];

$validMemberType = false;
$data = mysqli_query($conn, "SELECT memberType FROM membershipConfig WHERE memberType LIKE '$memberType' AND active = 1");
if (mysqli_num_rows($data) == 1) {
    $validMemberType = true;
}

if (!validate_text($_POST["memberType"], 'required') || !$validMemberType) {
    $output = "Please Select Membership";
} elseif (!validate_decimal_or_whole_number($_POST["userId"], 'required')) {
...
} elseif (!validate_text($_POST["gender"], 'required')) {
    $output = "Please Select A Gender";
} elseif (!validate_phone_number($_POST["contact"], 'required')) {
    $output = "Please Provide Valid 10-Digit<br>Phone number";
} elseif (!validate_text($_POST["country"], 'required')) {
    $output = "Please Provide Valid Country";
} elseif (!validate_text($_POST["town"], 'required')) {
    $output = "Please Provide Valid Town";
} elseif (!validate_textarea($_POST["address"], 'required')) {
