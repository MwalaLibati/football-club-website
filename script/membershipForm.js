    ...
    $.ajax({
      ...
      success: function (result) {
        ...
        if (result.indexOf("_____") !== -1) {
          ...
          let membershipPrice = $(".membershipPrice").val(); //get price
          let appliedForMemberType = $(".appliedForMemberType").val(); //get membership applying for
          $(".membershipMsg").html(
            '<h6 class="font-weight-bold text-success">Thank you for applying!</h6><p class="mb-4" id="business-type">Pay a subscription fee of K ' +
              membershipPrice +
              " to complete your <b>" +
              appliedForMemberType +
              '</b> membership application</p><br><button value="' +
              result +
              '" class="btn btn-success buyBtn">Pay Now</button><a href="index.php" class="btn btn-dark ml-2">Pay Later</a>'
          );
        } else {
