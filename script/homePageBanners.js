$(document).ready(function () {
  $(document).on("click", ".editBannerBtn", function () {
    ...
    $(".fileLabel").html("Change Banner Image");

    //get values to be edited
    let bannerId = $("input[type='hidden'][name='bannerId']").val();
    let priority = $("input[type='hidden'][name='priority']").val();
    let description1 = $("input[type='hidden'][name='description1']").val();
    let description2 = $("input[type='hidden'][name='description2']").val();

    //pre-populate model form on edit
  ...
  $(document).on("click", ".thumbnailImg", function () {
    ...
    //populate hidden fields with values to be edited/deleted later
    ...
    $("input[type='hidden'][name='description2']").val(description2);
    ...
    if (
      $("input[type='hidden'][name='bannerId']").val() &&
      $("input[type='hidden'][name='priority']").val() &&
      $(".deleteBannerBtn").val() &&
      src.length
    ) {
  ...
  $(document).on("click", ".deleteBannerBtn", function () {
    let btn = $(this);
    let bannerId = btn.val();
    let src = btn.data("src"); //data getter
    if (!src.length) {
    ...
    $.ajax({
      ...
      data: { type: "delete", bannerId: bannerId },
      beforeSend: function () {
      ...
      success: function (result) {
        ...
        if (result === "success") {
          ...
          $("img[src$='" + src + "'][name='mainImage']").attr(
            "src",
            "../images/placeholders/placeholder.png"
          );
          $(".overlayText").html(
            '<h2 class="overlay_description1">Main Heading</h2><p class="overlay_description2">Sub Heading...</p>'
          );
